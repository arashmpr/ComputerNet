...
class NotFoundHandler : public RequestPage
{
public:
    NotFoundHandler(string _path) : RequestPage(_path) {}
    Response *callback(Request *req);
};

Response *NotFoundHandler::callback(Request *req)
{
    Response *res = RequestPage::callback(req);
    res->set_code(404);
    return res;
}
...
Server *Server::create_server(int _port, std::string _address) {
    if (instance == nullptr)
...
void Server::run() {
  ...
  while (true) {
    acc_fd = accept(sc, (struct sockaddr *)&cli_addr, &clilen);
    if(acc_fd < 0)
      perror("Accept: ");
    Response *res = NULL;
    ...
    try {
      ...
      while (!req) {
        recv_len =
            recv(acc_fd, data + recv_total_len, BUFFER_SIZE - recv_total_len, 0);
        if (recv_len > 0) {
    ...
    delete res;
    int wr = send(acc_fd, res_data.c_str(), si, 0);
    if (wr != si)
      perror("Send:");
    close(acc_fd);
  }
...
RequestFileHandler::RequestFileHandler(string _path, string _type) {
  path = _path;
  type = _type;
}

Response *RequestFileHandler::callback(Request *req) {
...
RequestPage::RequestPage(string _path)
    : RequestFileHandler(_path, "text/" + get_extension(_path)) {}

RequestImage::RequestImage(string _path)
    : RequestFileHandler(_path, "image/" + get_extension(_path)) {}

RequestDocument::RequestDocument(string _path)
    : RequestFileHandler(_path, "application/" + get_extension(_path)) {}

RequestAudio::RequestAudio(string _path)
    : RequestFileHandler(_path, "audio/" + get_extension(_path)) {}
